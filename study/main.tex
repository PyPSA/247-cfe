...
\begin{frame}{Implementation of data center demand and supply \\
  todo}

  {\footnotesize
  The model optimizes a portfolio of carbon-free generation and storage technologies 
  procured by the participating C\&I consumers. The portfolio assets have to be located in the same market zone.
...
\begin{frame}{24/7 CFE matching: a case of inflexible consumer}
  ...
  must be higher or equal than a certain \alert{CFE score} $x$ multiplied with the total \alert{inflexible load} $d_t$:
  \vspace{0.1cm}
  \begin{equation}
...
{\footnotesize
  ...
  \begin{equation*}
  [1-f] \cdot d_t \le  d_t + (\overline{\delta} - \underline{\delta}) \le [1+f] \cdot d_t \quad \forall t \in T
  \label{eqn:range}
  \end{equation*}
  
  \vspace{0.1cm}
  \noindent where $\overline{\delta}, \underline{\delta} \in \mathbb{R}_{+}$ stand for positive/negative deviation of $\widetilde{d}_t$ and $d_t$ in hour $t$.
  \end{itemize}
...
\begin{frame}{Spatial load shifting problem 1/2}
  ...
  We introduce a concept of \alert{virtual links} as non-physical pathways for load shifts between data center nodes. \\
  ...
  \vspace{0.2cm}
  The nodal energy balance defined for inflexible consumers (eq. \ref{eqn:inflexnb}) is now extended by spatial load shifts, since the requested load at a given node can include shifts to/from other nodes: 
  \begin{equation}
  \sum_{r\in CFE} g_{r,t,n} + \sum_{s\in STO} \left(\bar{g}_{s,t,n} - \ubar{g}_{s,t,n}\right) - ex_{t,n} + im_{t,n}  = \textcolor{TUred}{d_{t,n} + \sum_{\vartheta \in \Theta_n^{rec}}\delta_{\vartheta, t} - \sum_{\vartheta \in \Theta_n^{snd}}\delta_{\vartheta, t}} \hspace{.5cm} \forall n \in N_{DC}, t \in T 
  \label{eqn:spatialnb}
  \end{equation}
  ...
  \begin{columns}
    \begin{column}{7cm}
      \centering
      \includegraphics[width=7.5cm]{images/Zavala_2021.png}
    \end{column}
  \begin{column}{5cm}
  \centering
  {\small
  \begin{circuitikz}
    \draw (0,13.5) to [short,i^=$im_{t,n}$]  (1.5,13.5) to (1.5,13);
    \draw [ultra thick] (0,13) node[anchor=south]{} -- (4,13);
    \draw(2.5,13) |- +(0,0.5) to [short,i^=$ex_{t,n}$] +(1.5,0.5);
    \draw (0.5,13) -- +(0,-0.5) node[sground]{};
    \draw (2,12) node[vsourcesinshape, rotate=270](V2){}
    (V2.left) -- +(0,0.6);
    \draw (3.5,13) -- (3.5,12.4);
    ...
    \draw (2,11.3) node{$g_{CFE,t,n}$};
    \draw (3.5,11.3) node{$g_{STO,t,n}$};
  \end{circuitikz}
  }
  \end{column}
  \end{columns}
...
\begin{frame}{Spatial load shifting problem 2/2}
  ...
  Flexibility constraints
      \begin{subequations}
        \begin{align}
            \widetilde{d_{n,t}} &=  d_{t,n} + \sum_{\vartheta \in \Theta_n^{rec}}\delta_{\vartheta, t} - \sum_{\vartheta \in \Theta_n^{snd}}\delta_{\vartheta, t} \quad \forall n \in N_{DC}, t \in T \label{eqn:spatiala} \\
            \widetilde{d_{n,t}} &\le [1+f] \cdot d_{t,n}  \quad \forall n \in N_{DC}, t \in T \label{eqn:spatialb} \\
            \widetilde{d_{n,t}} &\ge [1-f] \cdot d_{t,n}  \quad \forall n \in N_{DC}, t \in T \label{eqn:spatialc}
        \end{align}
        \label{eqn:spatialflex}
        \end{subequations}


  Adjusted CFE equation
  \vspace{0.1cm}
  \begin{equation}
    \begin{split}
  &\sum_{r\in CFE, t\in T} g_{r,t} + \sum_{s\in STO, t\in T} \left(\bar{g}_{s,t} - \ubar{g}_{s,t}\right) - \sum_{t\in T} ex_t + \sum_{t\in T} CFE_t \cdot im_t \geq \\ 
  &x_n \cdot \textcolor{TUred}{\sum_{t\in T} \left( d_{t,n} + \sum_{\vartheta \in \Theta_n^{rec}}\delta_{\vartheta, t} - \sum_{\vartheta \in \Theta_n^{snd}}\delta_{\vartheta, t}\right)} \quad \forall n \in N_{DC} \label{eqn:spatialCFE}
    \end{split}
  \end{equation}
  }
...
\begin{frame}{Spatially-temporal load shifting problem}
