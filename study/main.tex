...
\begin{frame}{Key findings}
  ...
  {\footnotesize
    ...
    \begin{enumerate}
      ...
      \item Space-time load-shifting flexibility also enables taking advantage of differences in local resources, harvesting renewable electricity in the best locations and indirectly \alert{opening access to it for all locations}. When paired with long-duration energy storage, the efficiency gains of this effect are even larger.
  \end{enumerate}
  }
...
\begin{frame}{Study design: implementing space-time load shifting}
...
\begin{frame}{Optimal procurement strategy as a function of load flexibility (1)}
...
\begin{frame}{24/7~CFE costs as a function of load flexibility (1)}
...
\begin{frame}{Optimal procurement strategy as a function of load flexibility (2)}
...
\begin{frame}{24/7~CFE costs as a function of load flexibility (2)}
  \label{cfe98cost}
...
\begin{frame}{Optimal procurement strategy as a function of load flexibility (3)}
...
\begin{frame}{24/7~CFE costs as a function of load flexibility (3)}
...
%---------------------------------------- Net costs and isolated runs
%----------------------------------------

\begin{frame}{Economic efficiency of co-optimized space-time load flexibility}
...
\begin{frame}{Isolating values of spatial and temporal load shifting}
  ...
  {\footnotesize
  \vspace{-0.2cm}
  Let's switch our focus to the scenario with {\bf 98\% CFE score and technology palette~1}. Similarly, the plots below show results for isolated spatial (left) and temporal (right) load shifting. A notable difference is that the value of spatial flexibility is much smaller (18\% of cost savings), while the value of temporal flexibility is larger (14\%). At the 98\% CFE score, 24/7 consumer complements own portfolio of procured CFE technologies with imports of electricity from the regional grid (as shown \hyperlink{cfe98cost}{above}). Thus, temporal shifts are also \alert{responsive to local grid's carbon intensity}, what drives its rising value. Shifting loads across locations still delivers a larger chunk of efficiency gains, but the absolute gains drop with a smaller portfolio of the CFE resources and the option to occasionally rely grid imports.
...
\begin{frame}{Isolating values of spatial and temporal load shifting}
  ...
  Finally, the same perspective for the scenario with {\bf 100\% CFE score and palette~2}. 
  Once a LDES system is added to the technology mix of the 24/7 consumer, it 
  can store excess generation from variable renewable resources for extended periods. It helps matching demand with CFE around-the-clock with considerably fewer resources everywhere, including the resource-constrained locations. Nevertheless, co-optimization of spatial load management and LDES brings synergies. Lond duration storage helps harvesting renewable electricity in the best locations and spatial flexibility \textit{indirectly} opens access to the cheaper clean electricity for all locations. We discuss this effect in more detail in the analysis of {\bf \hyperlink{redistribution-p2}{efficient redistribution}} of data center loads.
...
\begin{frame}{Insights from time-series data for optimized space-time load shifts}
  ...
  The time-series data for space-time load shifts reveals additional insights. Let's take a look at the selected scenario (see bottom right for details). The plot below shows {\bf hourly spatial load shifts} for a data center in Ireland.\footnote{{\scriptsize These plots for all locations and scenarios are available in the \hrefc{tba}{online annex}}} {\bf Negative} values mapped to \textcolor{blue}{blue color} represent loads \enquote{received} from other locations in a given hour, while {\bf positive} values mapped to \textcolor{TUred}{red color} represent loads \enquote{sent} away. 

  \includegraphics[height=5.3cm]{../results/1H-IEDKDEFIPT-allflex/plots/2025/EU/p1/cfe100/heatmaps/40_spatial_shift_ireland.pdf}
  \vspace{0.3cm}

  \source{DC in IE zone -- 24/7 CFE 100\% -- palette~1 tech -- spatial \& temporal flex co-optimized -- 40\% flexible workloads}
  }
...
\begin{frame}{Insights from time-series data for optimized space-time load shifts}
  ...
  Another perspective on the {\bf hourly spatial load shifts} for the same scenario (i.e., the same model run) but another location: a data center in Germany. Similarly, the {\bf negative} values mapped to \textcolor{blue}{blue color} represent loads \enquote{received} from other locations in a given hour, while {\bf positive} values mapped to \textcolor{TUred}{red color} represent loads \enquote{sent} away. 

  \vspace{0.1cm}
  \includegraphics[height=5.3cm]{../results/1H-IEDKDEFIPT-allflex/plots/2025/EU/p1/cfe100/heatmaps/40_spatial_shift_germany.pdf}
  }
  
  \source{DC in DE zone -- 24/7 CFE 100\% -- palette~1 tech -- spatial \& temporal flex co-optimized -- 40\% flexible workloads}
\end{frame}
...
\begin{frame}{Insights from time-series data for optimized space-time load shifts}
  ...
  \begin{enumerate}
    \item {\bf A stochastic pattern:} some load shifts are caused by the \alert{differences in (stochastic) weather conditions} across data center locations that drive feed-in of renewable generators. These load shifts are notable by the \enquote{vertical stripes} of a sudden color change (i.e., directions of spatial shifts) in the heatmaps above.

    \vspace{0.1cm}
    In the {\bf \hyperlink{nb1-40}{annex}}, we provide energy balance plots illustrating an example of the weather event causing this behavior: a data center located in Ireland experiences a tough situation on 03-04 March, due to calm days in the region and low feed-in of the wind generators procured with PPAs. Imports from the regional grid do not help in this situation, because the local electricity mix is dirty in this period and data center's planned CFE score is 100\%. However, a data center located in Denmark experiences good wind conditions on 03-04 March and has excess generation of clean electricity from the procured porfolio. Thus, a load shift between the two data centers helps to resolve the situation. 
    
    \vspace{0.1cm}
    To cover the load with CFE around-the-clock in a scenario with limited load flexibility (10\% of flexible workloads instead of 40\%---also provided in the {\bf \hyperlink{nb1-10}{annex}}), a data center in Ireland has to procure a \alert{much bigger portfolio of solar PV and battery storage}.
...
\begin{frame}{Insights from time-series data for optimized space-time load shifts}
  ...
  \begin{enumerate}
    ...
    \vspace{0.1cm}
    The quality of local resources, i.e., the average capacity factors of wind or solar PV in a given region, translates into the levelised costs of electricity (LCOE) for renewable generators. When spatial load shifting is possible, a rational 24/7 consumer can adjust own procurement strategy by contracting generators in better locations (lower LCOE) and co-optimizing spatial loads shifts accordingly. 
    ...
    \vspace{0.1cm}
    In the {\bf \hyperlink{nb3}{annex}}, we provide the energy balance plot for a data center in Germany in the first week of May illustrating an example of the load shift with a clear daily profile (as can also be seen in the heatmap).
...
\begin{frame}{Insights from time-series data for optimized space-time load shifts}
  ...
  Let us switch our attention to the temporal load management (while staying with the same scenario, see bottom right for details). The plot below shows {\bf hourly temporal load shifts} for a data center in Ireland. Following the logic, in this case {\bf negative} values mapped to \textcolor{blue}{blue color} represent \enquote{increase} of a load, i.e., workloads are shifted to a given hour from other times, while {\bf positive} values mapped to \textcolor{TUred}{red color} represent \enquote{decrease} of a load, thus workloads are shifted away from a given hour to another time. 

  \vspace{0.1cm}
  \includegraphics[height=5.3cm]{../results/1H-IEDKDEFIPT-allflex/plots/2025/EU/p1/cfe100/heatmaps/40_temporal_shift_ireland.pdf}
  }
...
\begin{frame}{Insights from time-series data for optimized space-time load shifts}
  ...
  Another perspective on the {\bf hourly temporal load shifts} for the same location (data center in Ireland) but another CFE score: 98\%. The color mapping stays as above.

  \vspace{0.3cm}
  \includegraphics[height=5.3cm]{../results/1H-IEDKDEFIPT-allflex/plots/2025/EU/p1/cfe98/heatmaps/40_temporal_shift_ireland.pdf}
  }
  
  \source{DC in IE zone -- 24/7 CFE 98\% -- palette~1 tech -- spatial \& temporal flex co-optimized -- 40\% flexible workloads}
\end{frame}
...
\begin{frame}{Insights from time-series data for optimized space-time load shifts}
  ...
  \begin{enumerate}
    ...
    \vspace{0.1cm}
    At the 98\% CFE score, 24/7 consumer complements own portfolio of procured CFE technologies with imports of electricity from the regional grid (as shown {\bf \hyperlink{cfe98cost}{above}}). Thus, temporal shifts are optimized as a \alert{function of the regional grids' carbon intensity}, i.e., workloads are shifted to \enquote{greener} times. The carbon content of the regional grid correlates with a profile of solar PV feed-in, what gives the shape for the temporal flexibility utilization.

    \vspace{0.1cm}
    In the {\bf \hyperlink{nb1-40}{annex}}, we provide supplementary graphics illustrating this behavior. In the scenario with 98\% CFE score, a data center in Ireland covers a difficult period of calm winter days by relying on both spatial and temporal load flexibility. Temporal shifts have a clear daily pattern (as shown in the heatmap above).
...
\begin{frame}{Insights from time-series data for optimized space-time load shifts}
  ...
  We wrap up our time-series results section with a brief look at the {\bf hourly spatial load shifts} for the scenario when LDES is added to the technology mix. Spatial flexibility utilization has a more \enquote{binary} and complex pattern. This can be attributed to optimization across the nodes x time periods graph of all flexibility elements' dispatch decisions and the synergies among them. In particular, co-optimized utilization of spatial shifts and LDES system enables harvesting renewable electricity in the best renewable locations (mainly wind due to its high capacity factor) and providing an access to low-cost clean electricity across all locations.

  \includegraphics[height=5.3cm]{../results/1H-IEDKDEFIPT-allflex/plots/2025/EU/p2/cfe100/heatmaps/40_spatial_shift_ireland.pdf}
  }
  
  \source{DC in IE zone -- 24/7 CFE 100\% -- palette~2 tech -- spatial \& temporal flex co-optimized -- 40\% flexible workloads}
\end{frame}
...
\begin{frame}{Economically efficient redistribution of data center loads \\
  Tech-P1 -- CFE98\% (left), CFE100\% (right)}

  \begin{columns}[T]
  \begin{column}{6cm}
    \centering
    \includegraphics[width=5.5cm]{../results/1H-IEDKDEFIPT-allflex/plots/2025/EU/p1/cfe98/utilization_dc/utilization_dc.pdf}
  \end{column}

  \begin{column}{6cm}
    \centering
    \includegraphics[width=5.5cm]{../results/1H-IEDKDEFIPT-allflex/plots/2025/EU/p1/cfe100/utilization_dc/utilization_dc.pdf}  
  \end{column}
  \end{columns}
...
\begin{frame}{Economically efficient redistribution of data center loads \\
  ...
  \begin{column}{6cm}
    \centering
    \includegraphics[width=5.5cm]{../results/1H-IEDKDEFIPT-allflex/plots/2025/EU/p2/cfe98/utilization_dc/utilization_dc.pdf}
  \end{column}

  \begin{column}{6cm}
    \centering
    \includegraphics[width=5.5cm]{../results/1H-IEDKDEFIPT-allflex/plots/2025/EU/p2/cfe100/utilization_dc/utilization_dc.pdf}  
  \end{column}
  \end{columns}

\end{frame}

% Lond duration storage helps harvesting renewable electricity in the
% best locations and spatial flexibility indirectly opens access to the cheaper clean electricity for all locations. 
...
\begin{frame}{A remark on system-level emissions reduction}

  {\footnotesize 
  \begin{itemize}
  \vspace{-0.2cm}
  \item  In the \hrefc{https://zenodo.org/record/7180097}{previous study}, we showed that 24/7 carbon free energy matching results in a \alert{notable and systematic reduction of emissions} both for participating consumers and in a regional grid (system-level emissions). While preparing this study, we observed that   load flexibility \textit{per se} does not decrease emissions further below relative to the high 24/7~CFE procurement baseline; however, flexibility makes achieving high CFE targets \alert{more cost-effective}.

  \item This effect takes place because 24/7 participants rely \textit{more} on own portfolio of CFE resources and \textit{less} on grid imports with higher CFE scores.
  ...
  Test model runs done without 24/7 procurement constraint (eq. \ref{eqn:bothCFE}), i.e., simulating a case when data centers cover demand purely with grid purchases without any policy regarding the origin of electricity, result in load shifting to be actively dispatched as a function of the local grids' carbon intensity, which enables shifting flexible loads to greener times and locations. 
  These results align with the dedicated literature (e.g., \hrefc{https://arxiv.org/abs/2106.11750}{Radovanovic et al. (2021)}).
...
{\footnotesize
   \noindent\fbox{%
   \parbox{\textwidth}{%
     {\bf Conclusion 1:} Data centers can shift jobs in time and location. Space-time load-shifting flexibility enables \alert{better access to clean electricity} and creates \alert{more options} for consumers to match demand with carbon-free electricity around-the-clock. 
   }}

   \noindent\fbox{%
   \parbox{\textwidth}{%
     {\bf Conclusion 2:} Increasing the potential of demand flexibility facilitates the \alert{efficiency and affordability} of 24/7 CFE procurement. High CFE targets can be achieved with fewer resources. As a result, more data center operators, as well as businesses from commercial and industrial sectors with flexible demands, may be interested to join the 24/7 CFE campaign.
   }}

   \noindent\fbox{%
   \parbox{\textwidth}{%
     {\bf Conclusion 3:} Demand flexibility is especially \alert{helpful for resource-constrained locations} where hourly matching with 24/7~CFE is difficult.
   }}

   \noindent\fbox{%
   \parbox{\textwidth}{%
     {\bf Conclusion 4:} Space-time load-shifting flexibility facilitates \alert{economically efficient redistribution of (data center) loads}. Workloads are actively shifted across locations and times. Notably, even though each location can be a net \enquote{sender} or  \enquote{receiver} of load, none of the locations is exclusively a sender or receiver. Differences in weather conditions driving renewable feed-in result in each location sending and receiving load at some hour of the year.
   }}

   \noindent\fbox{%
   \parbox{\textwidth}{%
     {\bf Conclusion 5:} Space-time load-shifting flexibility also enables taking advantage of differences in local resources, harvesting renewable electricity in the best locations and indirectly \alert{opening access to it for all locations}. When paired with long-duration energy storage, the efficiency gains of this effect are even larger.
   }}
...
\begin{frame}{Hourly CFE score of supply from grid -- PT 2025}
  \vspace{.5cm}
  \includegraphics[height=5.5cm]{../results/1H-IEDKDEFIPT-spatial/plots/2025/EU/p1/cfe100/heatmaps/0_GridCFE_PT1 0.pdf}
\end{frame}
...
\begin{frame}{Data center CFE supply and demand}
  ...
  \source{DC in IE zone -- 24/7 CFE 100\% -- palette~1 tech -- spatial \& temporal flex co-optimized -- 10\% flexible workloads}
\end{frame}
...
\begin{frame}{Data center CFE supply and demand}
  ...
  \source{DC in DK1 zone -- 24/7 CFE 100\% -- palette~1 tech -- spatial \& temporal flex co-optimized -- 40\% flexible workloads}
\end{frame}
...
\begin{frame}{Data center CFE supply and demand}
  ...
  \source{DC in DK1 zone -- 24/7 CFE 100\% -- palette~1 tech -- spatial \& temporal flex co-optimized -- 10\% flexible workloads}
\end{frame}
...
\begin{frame}{Data center CFE supply and demand}
  ...
  \source{DC in IE zone -- 24/7 CFE 100\% -- palette~2 tech -- spatial \& temporal flex co-optimized -- 0\% flexible workloads}
\end{frame}
...
\begin{frame}{Data center CFE supply and demand}
  ...
  \source{DC in DK zone -- 24/7 CFE 100\% -- palette~2 tech -- spatial \& temporal flex co-optimized -- 40\% flexible workloads}
\end{frame}
...
\begin{frame}{Data center CFE supply and demand}
  ...
  \source{DC in DK zone -- 24/7 CFE 100\% -- palette~2 tech -- spatial \& temporal flex co-optimized -- 0\% flexible workloads}
\end{frame}
