...
\begin{frame}
  ...
  \begin{itemize}
    ...
    We also thank the \hrefc{https://pypsa.org/}{PyPSA team} and many contributors to the open-source energy system modelling ecosystem used for this study (see: \hrefc{https://github.com/PyPSA/PyPSA}{github.com/PyPSA}).
...
{\footnotesize
  \begin{itemize}
    \vspace{-0.2cm}
    \item This work is a follow-up to an earlier study written by the authors in October 2022 on the \hrefc{https://zenodo.org/record/7180097}{"System-level impacts of 24/7 carbon-free electricity procurement in Europe"}.\\
...
\begin{frame}{Executive summary: key findings}
...
\begin{frame}{Introduction}
  ...
  \begin{column}{9cm}
    ...
    electricity systems across the globe. Many public and private energy buyers join this effort. For example, more than 400 members of the \hrefc{https://www.there100.org/}{RE100 group} have committed to procure enough renewable energy to match 100\% of their electricity consumption on an annual basis.

    \item Fully decarbonizing electricity grids, however, requires covering demand with carbon-free energy at all times, not just during periods of abundant sunshine or wind. This challenge requires embracing \hrefc{https://www.iea.org/reports/advancing-decarbonisation-through-clean-electricity-procurement}{innovative strategies} for decarbonization. 
    ...
    with local carbon-free electricity sources around-the-clock.
...
\begin{frame}{Motivations}
  ...
  \item In reality, many participants of the 24/7~CFE movement have some degree of flexibility in their electricity consumption. This flexibility takes the form of various mechamisms for temporal demand management available for \hrefc{https://doi.org/10.1016/j.rser.2021.111963}{a wide range} of C\&I consumers.
  ...
  \begin{columns}[T]
    \begin{column}{9cm}
      \begin{itemize}
        \vspace{-0.1cm}
        \item
        Demand for digital services is rapidly growing. Since 2010, internet traffic has expanded \hrefc{https://www.iea.org/energy-system/buildings/data-centres-and-data-transmission-networks}{25-fold}. Global data center energy use represents now nearly \hrefc{https://doi.org/10.1126/science.aba3758}{1\% of final electricity demand} worldwide. 
...
\begin{frame}{A growing body of research}
  ...
  \item The unique characteristics of data centers as electricity consumers and the active interest of ICT sector companies in sustainable energy drive a growing interest in the research community. Among many other, \hrefc{https://doi.org/10.1016/j.simpat.2015.01.005}{Wang et al. (2015)}, \hrefc{https://doi.org/10.1016/j.jnca.2017.01.036}{Toosi et al. (2017)},
...
\begin{frame}{Focus of the study}
  ...
  \begin{itemize}
    ...
    \item For this purpose, we elaborate the mathematical model developed in the previous study, by including spatial and temporal demand flexibility provided by electricity consumers following 24/7~CFE goals. Thus, a flexible 24/7 participant could benefit from co-optimizing utilization of available demand flexibility (across space and/or time) and procurement strategies to match every kWh of electricity consumption with carbon-free energy around-the-clock \alert{more resource-efficient}.
...
\begin{frame}{Study design: data centers}
  ...
  \begin{itemize}
    ...
    \item Load shifts take place via \enquote{virtual links}. Virtual links form a \alert{complete graph}, i.e., every pair of data centers is connected by a unique link.
...
\begin{frame}{Study design: data center load flexibility}
  ...
  {\footnotesize
    ...
    \centering
    \vspace{0.2cm}
    \includegraphics[width=6cm]{images/drawio_flexrange_d.jpg}
    \end{column}
...
\begin{frame}{Study design: space-time load shifting}
  ...
  \begin{itemize}
    \vspace{0.1cm}
    \item To capture spatial flexibility, we model a spatial load management system that can shift load across data center locations via virtual links.
    \item To capture temporal flexibility, we model a load scheduling system that can shift load of a data center over time.
...
\begin{frame}{Study design: scenario space}
  ...
  -- \alert{Palette~2} includes all above plus Long Duration Energy Storage (LDES) system. \\
  Technology assumptions are provided in \alert{\hyperlink{datasources}{Annex B: Tools and data sources}}. 
 
  \item For interested parties, we publish an \hrefc{https://doi.org/10.5281/zenodo.8185850}{online Annex} with a full pack of modelling results alongside this study. The materials include modelling results for all scenario combinations in a form of plots and summary CSV files.
...
\begin{frame}{Study design: limitations}
  ...
  \item This study is done in a spirit of \alert{modelling for insight} rather than \alert{modelling for numbers}. The design of this study does not aim at quantifying the real-life benefits of demand side flexibility for data centers. It is rather a model experiment to explore \textit{how and why} flexibility of demand can be beneficial for achieving 24/7 carbon-free energy goals. The results we present should thus be viewed with a fair degree of caution, i.e., as a modelling-based insight rather than quantitative projection. 
  
  \item Quantifying the actual costs and benefits for the ICT industry of utilizing demand flexibility requires \alert{additional empirical research}. Further studies could usefully explore the costs and technical potentials of achieving a certain share of flexible workloads, which are \alert{not considered} in this study. Thus, a range of flexible loads is fixed per scenario and flexibility utilization (i.e., shifting of loads in space and time) is modeled as a \enquote{zero-cost} variable. Including information on implicit flexibility costs would help to quantify the flexibility benefits with a greater degree of accuracy. Another empirical improvement could address technical aspects and properties of flexible workloads, such as physical constraints associated with quick ramping of power usage up/down, reliability \& performance constraints, etc. Further research is needed to capture the promising role of clean firm generation technologies in achieving 24/7 CFE goals with some degree of spatial and/or temporal load flexibility. This case is particularly relevant, since the authors' \hrefc{https://zenodo.org/record/7180097}{previous modeling work} demonstrated that 24/7 CFE procurement could create an early market and drive deployment of advanced technologies, such as LDES and clean firm generation.
...
\label{sec:efficiency}
  ...
  \begin{column}{7cm}
    A plot on the left shows the \alert{total annual costs [\euro/a]} for achieving 24/7~CFE policy in all locations (left y-axis) and their relative representation as a percentage of the zero flexibility scenario's costs (right y-axis). The costs are plotted as a function of the load flexibility potential. The values represent the total procurement costs, not a 24/7 "premium" (the additional costs to the price of electricity in a local market).
...
\label{ssec:redistribution}
  ...
  \vspace{0.3cm}
  The analysis of the time-series data for space-time load shifts revealed interesting insights. However, a natural question to ask based on the results above is whether space-time flexibility also leads to the \textit{net shift} of loads? In other words: \textit{Does load flexibility facilitate a redistribution of the average utilization of data centers?}
...
\begin{frame}{Further remarks}
  ...
  \item This effect takes place because 24/7 participants with higher CFE scores rely more on their own portfolio of CFE resources and less on grid imports.
...
{\footnotesize
   \noindent\fbox{%
   \parbox{\textwidth}{%
     {\bf Conclusion 1:} Demand flexibility enables \alert{better access to clean electricity} and creates \alert{more options} for consumers to match demand with carbon-free electricity around-the-clock. 
   }}

   \noindent\fbox{%
   \parbox{\textwidth}{%
     {\bf Conclusion 2:} Some flexible electricity consumers, such as data centers, can shift computing jobs and associated power loads in both time and location. These mechanisms facilitate the \alert{efficiency and affordability} of 24/7 CFE procurement. The co-optimized space-time load-shifting can reduce the costs of 24/7~CFE by up to 34\%, depending on the level of flexibility and technologies available.
   }}
   ...
   \noindent\fbox{%
   \parbox{\textwidth}{%
     {\bf Conclusion 4:} Space-time load-shifting facilitates \alert{economically efficient redistribution of loads} to locations with good carbon-free resources. When paired with long-duration energy storage, the efficiency gains of this effect are even larger.
   }}

   \noindent\fbox{%
   \parbox{\textwidth}{%
     {\bf Conclusion 5:} In the European energy system, the hourly profiles of wind power generation have a low correlation over long distances due to different weather conditions. 
     Spatial load flexibility enables the system to move load to locations when and where there is high wind generation, thus \alert{saving costs of energy storage} and \alert{reducing curtailment} of excess generation.      
   }}
...
\begin{frame}\frametitle{\quad}
  ...
  {\small
  \vspace{.2cm}
  This study is done in a spirit of open and reproducible research: \\
  ...
  \faLink~\hrefc{https://doi.org/10.5281/zenodo.8185850}{https://doi.org/10.5281/zenodo.8185850} \\
  ...
  \noindent\fbox{%
  \parbox{\textwidth}{%
    {\bf Suggested citation:} Riepin, I. \& Brown, T., \textit{The value of space-time load-shifting flexibility for 24/7 carbon-free electricity procurement}, Department of Digital Transformation in Energy Systems TU Berlin, 26 July 2023. Zenodo. DOI: doi.org/10.5281/zenodo.8185850
  }}
  }
\end{frame}
...
\begin{frame}{Spatial load shifting problem 1/3}
  ...
  We introduce a concept of \alert{spatial load management system} that allows for shifting load across locations. The load shifts take place via \alert{virtual links} -- ICT-based pathways between data centers.
...
\begin{frame}{Spatial load shifting problem 2/3}
  ...
  We introduce a concept of \alert{spatial load management system} that allows for shifting workloads across locations. The load shifts take place via \alert{virtual links} -- ICT-based pathways between data centers.

  Here we follow a mathematical formulation of virtual links proposed by \href{https://doi.org/10.1016/j.apenergy.2022.119930}{Zhang \& Zavala (2022)}. Let $\Theta$ be the set of all virtual links; let $\delta_\vartheta \in \mathbb{R}_{+}$ be load shifts (flows via virtual pathways); and let $N_{DC}$ be the set of data centers (flexible consumers). 
  We can define $\Theta_n^{snd} := \{\vartheta \in \Theta | snd(\vartheta) = n\} \subseteq \Theta$ and $\Theta_n^{rec} := \{\vartheta \in \Theta | rec(\vartheta) = n\} \subseteq \Theta$ to be the set of sending and receiving virtual links at node $n \in N_{DC}$. 
...
\begin{frame}{PyPSA: an energy modelling ecosystem}
  \label{datasources} 
\begin{columns}[T]
\begin{column}{7cm}
...
\begin{frame}{Economic efficiency of co-optimized space-time load shifts\\
  98\% CFE and 100\% CFE w/ LDES}
  \label{eff-two}

  {\footnotesize

  The two plots below complement the discussion on \hyperlink{isolated-hook}{"Isolated values of spatial and temporal load shifts"}. The plots show the \alert{total annual costs [\euro/a]} for achieving 24/7~CFE policy in all locations (left y-axis) and their relative representation as a percentage of the zero flexibility scenario's costs (right y-axis). The costs are plotted as a function of the load flexibility potential.\\
  {\bf Left panel}: 98\% CFE scenario; {\bf right panel}: 100\% CFE w/ LDES scenario;

  \begin{columns}
    \begin{column}{7cm}
    \includegraphics[width=6.8cm]{../results/1H-IEDKDEFIPT-allflex/plots/2025/EU/p1/cfe98/ci_abs_costs.pdf}
    \end{column}
    
    \begin{column}{7cm}
    \includegraphics[width=6.8cm]{../results/1H-IEDKDEFIPT-allflex/plots/2025/EU/p2/cfe100/ci_abs_costs.pdf}
    \end{column}
  \end{columns}
