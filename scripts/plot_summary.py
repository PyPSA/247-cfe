...
def system_capacity_diff():
    ...
    gens = df.loc[["system_inv_" + t for t in exp_generators]].rename(
        {"system_inv_" + t: t for t in exp_generators}
    )
    links = df.loc[["system_inv_" + t for t in exp_links]].rename(
        {"system_inv_" + t: t for t in exp_links}
    )
    dischargers = df.loc[["system_inv_" + t for t in exp_dischargers]].rename(
        {"system_inv_" + t: t for t in exp_dischargers}
    )
    chargers = df.loc[["system_inv_" + t for t in exp_chargers]].rename(
        {"system_inv_" + t: t for t in exp_chargers}
    )
    chargers = chargers.drop(
        ["battery_charger-%s" % year]
    )  # display only battery discharger capacity
    ...
    fig.tight_layout()
    fig.savefig(
        snakemake.output.plot.replace("capacity.pdf", "system_capacity_diff.pdf"),
        transparent=True,
    )
...
def system_curtailment():
    ...
    ldf = df.loc[["system_curtailment_" + t for t in existing_rows]].rename(
        {"system_curtailment_" + t: t for t in existing_rows}
    )
    ldf = ldf.xs(locations[0], axis="columns", level=1)
    ...
    fig.tight_layout()
    fig.savefig(
        snakemake.output.plot.replace("capacity.pdf", "system_curtailment.pdf"),
        transparent=True,
    )
...
def utilization_dc(names, flexibilities):
    ...
    for flex in flexibilities:
        ...
        if snakemake.config["ci"]["spatial_shifting"] == True:
            for node in names:
                ...
                mean_flex.append(
                    -df.mean()
                )  # negative spatial shift implies increasing load
    ...
    axs[-1].set_xticks(
        [int(f) for f in flexibilities]
    )  # Set xticks positions using integers
    axs[-1].set_xticklabels(
        [f"{f}%" for f in flexibilities]
    )  # Set xticks labels with percentage signs
