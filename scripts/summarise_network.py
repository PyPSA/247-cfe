...
def summarise_network(n, policy, tech_palette):
    ...
    for location, name in datacenters.items():
        ...
        used_grid = (-n.links_t.p1[name + " import"].multiply(weights,axis=0))*grid_cfe
        ...
        results[f'{location}']['ci_clean_used_total']  = used_grid.sum() + used_local.sum()
        results[f'{location}']['ci_clean_excess_total']  = excess.sum()
        results[f'{location}']['ci_demand_total']  = p_demand.sum()

        results[f'{location}']['ci_fraction_clean'] = results[f'{location}']['ci_clean_total']/results[f'{location}']['ci_demand_total']
        results[f'{location}']['ci_fraction_clean_used_local'] = results[f'{location}']['ci_clean_used_local_total']/results[f'{location}']['ci_demand_total']
        results[f'{location}']['ci_fraction_clean_used_grid'] = results[f'{location}']['ci_clean_used_grid_total']/results[f'{location}']['ci_demand_total']
        results[f'{location}']['ci_fraction_clean_used'] = results[f'{location}']['ci_clean_used_total']/results[f'{location}']['ci_demand_total']
        results[f'{location}']['ci_fraction_clean_excess'] = results[f'{location}']['ci_clean_excess_total']/results[f'{location}']['ci_demand_total']
        ...
        results[f'{location}']['system_emission_rate'] = hourly_emissions.sum() / load.sum()
        ...
        rest_system_buses = n.buses.index[~n.buses.index.str.contains(name) & ~n.buses.index.str.contains(location)]
        ...
        line_imp_subsetA = n.lines_t.p1.loc[:,n.lines.bus0.str.contains(location)].sum(axis=1)
        line_imp_subsetB = n.lines_t.p0.loc[:,n.lines.bus1.str.contains(location)].sum(axis=1)
        line_imp_subsetA[line_imp_subsetA < 0] = 0.
        line_imp_subsetB[line_imp_subsetB < 0] = 0.

        links_imp_subsetA = n.links_t.p1.loc[:,n.links.bus0.str.contains(location) & 
                            (n.links.carrier == "DC") & ~(n.links.index.str.contains(name))].sum(axis=1)
        links_imp_subsetB = n.links_t.p0.loc[:,n.links.bus1.str.contains(location) & 
                            (n.links.carrier == "DC") & ~(n.links.index.str.contains(name))].sum(axis=1)
        ...
        results[f'{location}']['emissions_zone'] = (country_hourly_emissions*weights).sum()/1e6


        # 4: Storing invested capacities at CI node

        for tech in clean_techs:
            results[f'{location}']['ci_cap_' + tech] = n.generators.at[name + " " + tech,"p_nom_opt"]
        ...
        for charger in storage_charge_techs:
            ...
            if name + " " + charger in n.links.index:
                results[f'{location}']['ci_cap_' + charger.replace(' ', '_')] = n.links.at[name + " " + charger, "p_nom_opt"]*temp['eff_' + charger]
            else:
                results[f'{location}']['ci_cap_' + charger.replace(' ', '_')] = 0.
        ...
        for discharger in storage_discharge_techs:
            ...
            if name + " " + discharger in n.links.index:
                results[f'{location}']['ci_cap_' + discharger.replace(' ', '_')] = n.links.at[name + " " + discharger, "p_nom_opt"]*temp['eff_' + discharger]
            else:
                results[f'{location}']['ci_cap_' + discharger.replace(' ', '_')] = 0.

        # Storing generation at CI node  
        
        for tech in clean_techs:
            results[f'{location}']['ci_generation_' + tech] = n.generators_t.p[name + " " + tech].multiply(weights,axis=0).sum()

        for discharger in storage_discharge_techs:
            if name + " " + discharger in n.links.index:
                results[f'{location}']['ci_generation_' + discharger.replace(' ', '_')] = - n.links_t.p1[name + " " + discharger].multiply(weights,axis=0).sum()
            else:
                results[f'{location}']['ci_generation_' + discharger.replace(' ', '_')] = 0.
        ...
        for gen in exp_generators:
            ...
            results[f'{location}']['system_inv_' + gen.replace(' ', '_')] = temp['system_optcap_' + gen] - system_gens.loc[system_gens.index.str.contains(f'{gen}'), 'p_nom'].sum()
        ...
        for link in exp_links:
            ...
            results[f'{location}']['system_inv_' + link] = temp['system_optcap_' + link] - system_links.loc[system_links.index.str.contains(f'{link}'), 'p_nom'].sum()
        ...
        for charger in exp_chargers:
            ...
            results[f'{location}']['system_inv_' + charger.replace(' ', '_')] = temp['system_optcap_' + charger] - system_chargers.loc[system_chargers.index.str.contains(f'{charger}'), 'p_nom'].sum()
            results[f'{location}']['system_inv_' + charger.replace(' ', '_')] *= temp['eff_' + charger]
        ...
        for discharger in exp_dischargers:
            ...
            results[f'{location}']['system_inv_' + discharger.replace(' ', '_')] = temp['system_optcap_' + discharger] - system_dischargers.loc[system_dischargers.index.str.contains(f'{discharger}'), 'p_nom'].sum()
            results[f'{location}']['system_inv_' + discharger.replace(' ', '_')] *= temp['eff_' + discharger]

        # 6: Storing costs at CI node

        total_cost = 0.
        for tech in clean_techs:
            results[f'{location}']['ci_capital_cost_' + tech] = results[f'{location}']['ci_cap_' + tech]*n.generators.at[name + " " + tech,"capital_cost"]
            results[f'{location}']['ci_marginal_cost_' + tech] = results[f'{location}']['ci_generation_' + tech]*n.generators.at[name + " " + tech,"marginal_cost"]
            cost =  results[f'{location}']['ci_capital_cost_' + tech] + results[f'{location}']['ci_marginal_cost_' + tech]
            results[f'{location}']['ci_cost_' + tech] = cost
            total_cost += cost

        results[f'{location}']['ci_capital_cost_grid'] = 0
        results[f'{location}']['ci_marginal_cost_grid'] = (n.links_t.p0[name + " import"]*weights*n.buses_t.marginal_price[location]).sum()
        cost =  results[f'{location}']['ci_capital_cost_grid'] + results[f'{location}']['ci_marginal_cost_grid']
        ...
        results[f'{location}']['ci_revenue_grid'] = (n.links_t.p0[name + " export"]*weights*n.buses_t.marginal_price[location]).sum()
        results[f'{location}']['ci_average_revenue'] =  results[f'{location}']['ci_revenue_grid'] / results[f'{location}']['ci_demand_total']

        if "battery" in storage_techs:
            results[f'{location}']['ci_capital_cost_battery_storage'] = n.stores.at[f"{name} battery","e_nom_opt"]*n.stores.at[f"{name} battery","capital_cost"]
            results[f'{location}']['ci_cost_battery_storage'] = results[f'{location}']['ci_capital_cost_battery_storage']
            total_cost += results[f'{location}']['ci_cost_battery_storage']

            results[f'{location}']['ci_capital_cost_battery_inverter'] = n.links.at[f"{name} battery charger","p_nom_opt"]*n.links.at[f"{name} battery charger","capital_cost"]
            results[f'{location}']['ci_cost_battery_inverter'] = results[f'{location}']['ci_capital_cost_battery_inverter']
            total_cost += results[f'{location}']['ci_cost_battery_inverter']
        else:
            results[f'{location}']['ci_capital_cost_battery_storage'] = 0.
            results[f'{location}']['ci_cost_battery_storage'] = 0.
            results[f'{location}']['ci_capital_cost_battery_inverter'] = 0.
            results[f'{location}']['ci_cost_battery_inverter'] = 0.

        if "hydrogen" in storage_techs:
            results[f'{location}']['ci_capital_cost_hydrogen_storage'] = n.stores.at[f"{name} H2 Store","e_nom_opt"]*n.stores.at[f"{name} H2 Store","capital_cost"]
            results[f'{location}']['ci_cost_hydrogen_storage'] = results[f'{location}']['ci_capital_cost_hydrogen_storage']
            total_cost += results[f'{location}']['ci_cost_hydrogen_storage']

            results[f'{location}']['ci_capital_cost_hydrogen_electrolysis'] = n.links.at[f"{name} H2 Electrolysis","p_nom_opt"]*n.links.at[f"{name} H2 Electrolysis","capital_cost"]
            results[f'{location}']['ci_cost_hydrogen_electrolysis'] = results[f'{location}']['ci_capital_cost_hydrogen_electrolysis']
            total_cost += results[f'{location}']['ci_cost_hydrogen_electrolysis']

            results[f'{location}']['ci_capital_cost_hydrogen_fuel_cell'] = n.links.at[f"{name} H2 Fuel Cell","p_nom_opt"]*n.links.at[f"{name} H2 Fuel Cell","capital_cost"]
            results[f'{location}']['ci_cost_hydrogen_fuel_cell'] = results[f'{location}']['ci_capital_cost_hydrogen_fuel_cell']
            total_cost += results[f'{location}']['ci_cost_hydrogen_fuel_cell']
        else:
            results[f'{location}']['ci_capital_cost_hydrogen_storage'] = 0.
            results[f'{location}']['ci_cost_hydrogen_storage'] = 0.
            results[f'{location}']['ci_capital_cost_hydrogen_electrolysis'] = 0.
            results[f'{location}']['ci_cost_hydrogen_electrolysis'] = 0.
            results[f'{location}']['ci_capital_cost_hydrogen_fuel_cell'] = 0.
            results[f'{location}']['ci_cost_hydrogen_fuel_cell'] = 0.

        results[f'{location}']["ci_total_cost"] = total_cost
        results[f'{location}']["ci_average cost"] = results[f'{location}']['ci_total_cost']/results[f'{location}']['ci_demand_total']

        # 7: Other calculations

        results[f'{location}']['zone_average_marginal_price'] = n.buses_t.marginal_price[location].sum() / len(n.snapshots)
        ...
        if snakemake.config['global']['policy_type'] == "co2 cap":
            ...
            results[f'{location}']["co2_price"] = snakemake.config['global'][f'co2_price_{year}']
