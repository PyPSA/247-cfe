...
def summarise_scalars():
    ...
    for fn in fns:
        ...
        for location in locations:
            ...
            float_keys = [k for k in results[f'{location}'].keys() if isinstance(results[f'{location}'][k],float)]
            df.loc[:, (scenario,location)] = pd.Series({k : results[f'{location}'][k] for k in float_keys})
